...
try {
    ...
    // Fetch all departments handled by the logged-in dean
    ...
    if (empty($departmentIds)) {
        throw new Exception('No departments found for the logged-in dean.');
    }

    // Convert department IDs into a comma-separated string
    $departmentIdsStr = implode(',', $departmentIds);
    ...
    // Query students associated with the dean's departments
    $studentQuery = "
        SELECT 
            users.user_id, 
            users.first_name, 
    ...
    // Get total number of rows for pagination purposes
    $countQuery = "
        SELECT COUNT(*) AS total
        FROM users
        INNER JOIN department ON users.department_id = department.department_id
