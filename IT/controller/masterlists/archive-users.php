...
if (isset($_POST['delete_user_id'])) {
    ...
    try {
        ...
        // Archive supervisor evaluations
        $stmt = $conn->prepare("INSERT INTO archive_db.supervisor_evaluations SELECT * FROM aims_db.supervisor_evaluations WHERE evaluator_id = ? OR student_id = ?");
        $stmt->bind_param("ii", $userIdToDelete, $userIdToDelete);
        $stmt->execute();
        ...
        // Delete from student_supervisor in aims_db
        $stmt = $conn->prepare("DELETE FROM aims_db.student_supervisor WHERE id = ?");
        $stmt->bind_param("i", $idToArchive);
        if (!$stmt->execute()) {
            echo "Error deleting student_supervisor data: " . $stmt->error;
        }
        $stmt->close();

        // Delete submit_requirements for the student_id from aims_db
        $stmt = $conn->prepare("DELETE FROM aims_db.submit_requirements WHERE student_id = ?");
        $stmt->bind_param("i", $userIdToDelete);
        $stmt->execute();

        // Optionally delete requirements if necessary (adjust query as needed)
        $stmt = $conn->prepare("
        ...
        // Now delete related coordinator evaluations from aims_db before deleting the user
        $stmt = $conn->prepare("DELETE FROM aims_db.supervisor_evaluations WHERE evaluator_id = ? OR student_id = ?");
        $stmt->bind_param("ii", $userIdToDelete, $userIdToDelete);
        $stmt->execute();

        // Delete related weekly reports from aims_db before deleting the user
        $stmt = $conn->prepare("DELETE FROM aims_db.weekly_reports WHERE student_id = ?");
        $stmt->bind_param("i", $userIdToDelete);
        $stmt->execute();

        // Delete related ojt_hours data from aims_db before deleting the user
