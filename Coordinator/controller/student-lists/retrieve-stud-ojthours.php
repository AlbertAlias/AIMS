<?php 
    ...
    //     $searchTerm = "%$search%";
    //     $stmt->bind_param('issii', $user_id, $searchTerm, $searchTerm, $start, $length);
    //     $stmt->execute();
    ...
    //     $totalStmt->bind_param('is', $user_id, $searchTerm);
    //     $totalStmt->execute();
...
try {
    ...
    $sumHoursSql = "SELECT SUM(total_hours) AS total_hours_sum FROM ojt_hours WHERE student_id = ?";
    $sumHoursStmt = $conn->prepare($sumHoursSql);
    $sumHoursStmt->bind_param('i', $user_id);
    $sumHoursStmt->execute();
    $sumHoursResult = $sumHoursStmt->get_result();
    $totalHoursSum = $sumHoursResult->fetch_assoc()['total_hours_sum'] ?: 0;
...
if (isset($sumHoursStmt) && $sumHoursStmt) {
    $sumHoursStmt->close();
}
$conn->close();
