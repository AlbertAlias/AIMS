<?php
    ...
    if (isset($_SESSION['user_type']) && $_SESSION['user_type'] === 'Coordinator') {
        ...
        // SQL query to fetch rejected requirements from students within the same department
        $sql = "
            SELECT 
                sr.submit_id,
            ...
            WHERE 
                u.department_id = ? AND sr.status = 'rejected'
        ";
        ...
        if ($result->num_rows > 0) {
            ...
            error_log('No rejected requirements found for department_id ' . $department_id);
            echo json_encode(['status' => 'error', 'message' => 'No rejected requirements found.']);
        }
