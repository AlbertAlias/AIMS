document.addEventListener('DOMContentLoaded', () => {
    ...
    function fetchReports() {
        fetch('./controller/weekly-reports/get_interns.php')
    ...
    window.viewReport = function(student_id) {
        fetch(`controller/weekly-reports/get_report.php?studentId=${student_id}`)
            .then(response => response.json())
            .then(data => {
    ...
    function handleReportAction(student_id, action) {
        fetch('controller/weekly-reports/update_report_status.php', {
            method: 'POST',
