...
try {
    ...
    // Query to get filtered and paginated data, only for 'intern' user type
    $sql = "SELECT id, employee_no, last_name, first_name, middle_name, gender, personal_email, username, user_type, department_id 
        FROM users 
        WHERE user_type = 'intern' AND CONCAT_WS(' ', first_name, last_name, employee_no, personal_email, username, user_type) LIKE ? 
        LIMIT ?, ?";
    ...
    // Query to get total count for pagination, considering only 'intern' user type
    $totalSql = "SELECT COUNT(*) AS total FROM users WHERE user_type = 'intern' AND CONCAT_WS(' ', first_name, last_name, employee_no, personal_email, username, user_type) LIKE ?";
